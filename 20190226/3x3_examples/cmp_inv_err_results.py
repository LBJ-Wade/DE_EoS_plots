...
EoS_dir = 'post_no_extra_w_inv_err/EoS'
EoS_dir2 = 'post_no_extra_w_inv_err/EoS.bak'
...
for i in range(N1):
    for j in range(N2):
        ...
        w = loadtxt(EoS_dir2+'/eos_'+str(cnt+dcnt)+'.txt')
        ax.fill_between(z,y1=w[:,2],y2=w[:,3],color=colors[1],alpha=0.25)
        ax.plot(z,w[:,2],color=colors[1],ls='-',lw=1)
        ax.plot(z,w[:,3],color=colors[1],ls='-',lw=1)
        ax.plot(z,w[:,0],color=colors[1],ls='--',lw=2)
