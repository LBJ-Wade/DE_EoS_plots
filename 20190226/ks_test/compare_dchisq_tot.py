...
hist(dchi2_tot_f-dchi2_tot,bins=30,density=True,alpha=0.5)
hm = mean(dchi2_tot_f-dchi2_tot)
h,b=histogram(dchi2_tot_f-dchi2_tot,bins=30,density=True)

b = (b[1:]+b[0:-1])/2

#plot(b,h,'b-.',label=r'Dist of $|\Delta\chi^2_{\rm \Lambda JDF^{\ast}}|-|\Delta\chi^2_{\rm \Lambda JD16^{\ast}}|$')
vlines(hm,ymin=0,ymax=1.05*max(h),linestyles='solid',lw=5,color='r',alpha=0.5,label=r'Mean value = $'+str(round(hm,2))+'$')
